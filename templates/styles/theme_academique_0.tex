...
\newcommand{\TitreIndispensableDansBarrePath}[1]{%
  \pgfmathsetmacro{\ArrowLen}{#1*1.5}%
  \draw[
    -{Latex[length=#1 pt, width=#1 pt]},
    color=white,
    line width={#1*0.4},
    line cap=round
  ]
...
%
% #1 = Niveau (1 à 5)
% #2 = Couleur des étoiles pleines (= couleur du titre / de la barre)
...
\newcommand{\TitreEtoilesDansBarrePath}[3]{%
  ...
  \foreach \i in {1,...,5}{%
    ...
    \ifnum\i>#1
      % Étoile vide : contour blanc, intérieur blanc
      \node[
    ...
    \else
      % Étoile pleine : contour blanc, remplie de la couleur du titre
      \node[
        star, star points=5, star point ratio=.50, rounded corners=0.2pt,
        minimum size=\TailleEtoile pt, inner sep=0pt, outer sep=0pt,
...
\newcommand{\TitreMetaBarreGauche}[2]{%
  ...
  % DIAGNOSTIC TEMPORAIRE -- à retirer une fois le bug de la flèche
  % résolu. Regarde dans le fichier .log la ligne "DIAGNOSTIC
  % indispensable=[...]" : si tu vois autre chose que "True" ou
  % "true" entre les crochets (ex: un espace, un retour à la ligne,
  % "1", etc.), ça nous dira exactement quoi comparer en plus.
  \typeout{DIAGNOSTIC indispensable=[\TitreTmpInd]}%
  ...
  \ifstrequal{\TitreTmpInd}{true}{\TitrePedIndispensabletrue}{}%
  \ifstrequal{\TitreTmpInd}{True}{\TitrePedIndispensabletrue}{}%
  \ifstrequal{\TitreTmpInd}{1}{\TitrePedIndispensabletrue}{}%
  % DIAGNOSTIC TEMPORAIRE
  \typeout{DIAGNOSTIC IndispensableBool=[\ifTitrePedIndispensable true\else false\fi]}%
...
\newcommand{\TitrePedagogiqueAvecMeta}[7]{%
  ...
  \fi
  % DIAGNOSTIC TEMPORAIRE
  \typeout{DIAGNOSTIC TitreTropLong=[\ifTitrePedTropLong true\else false\fi] largeurTitreNat=[\the\wd\TitreMesureTitre] largeurMax=[\the\dimexpr#3\relax]}%
  ...
  %
  % ⚠️ CHANGEMENT : l'ancienne mesure utilisait \hbox{#2 #7}, qui ne
  % peut JAMAIS retourner à la ligne (largeur infinie) -- elle ne
  % pouvait donc que dire "tient en une ligne ?" oui/non, jamais
  % calculer la largeur réelle d'un titre qui doit passer sur 2+
  % lignes (elle retombait alors bêtement sur #3, la largeur MAX,
  % même si le texte replié aurait pu tenir dans moins large).
  %
  % \varwidth{#3} (package déjà chargé dans le projet) résout ça
  % nativement : il replie le texte dans la limite #3 si besoin,
  % et renvoie la largeur la plus ÉTROITE qui contient le résultat
  % (= largeur naturelle si une seule ligne suffit, ou largeur de la
  % ligne repliée la plus longue sinon). \TitreLargeurEffLen suit
  % donc TOUJOURS la largeur réelle du bloc de texte, qu'il tienne
  % sur une ou plusieurs lignes -> le trait s'ajuste bien dans tous
  % les cas, plus besoin du \ifdim de comparaison à #3.
  % --------------------------------------------------------------
  \sbox{\TitreMesureTitre}{\begin{varwidth}{#3}#2#7\end{varwidth}}%
  \setlength{\TitreLargeurEffLen}{\wd\TitreMesureTitre}%
  % DIAGNOSTIC TEMPORAIRE
  \typeout{DIAGNOSTIC LargeurEffLen(varwidth)=[\the\TitreLargeurEffLen] largeurMax(#3)=[\the\dimexpr#3\relax]}%
  ...
  % DIAGNOSTIC TEMPORAIRE
  \typeout{DIAGNOSTIC difficulte=[\TitreTmpDiffCheck] EtoilesGaucheOui=[\ifTitrePedEtoilesGauche true\else false\fi]}%
  \ifTitrePedTropLong
...
\NewDocumentEnvironment{encadrestylezero}{mmm}
{%
  \begin{tcolorbox}[
    ...
    overlay={%
      ...
      \node[
        rotate=90,
        anchor=south,
        font={\sffamily\bfseries\fontsize{5pt}{6pt}\selectfont},
        text=#3,
        inner sep=0pt
      ...
      \node[
        rotate=90,
        anchor=center,
        font={\sffamily\bfseries\small\fontsize{5pt}{6pt}\selectfont},
        text=white,
        inner sep=0pt
